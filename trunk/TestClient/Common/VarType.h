...
public: // constructor
	...
	VarType(const VarType& obj)	: m_int(obj.m_int),m_nType(obj.m_nType)					{ if(obj.m_ptr) m_ptr=new MyString(*obj.m_ptr); else m_ptr=NULL; }
	VarType(const char* str,TYPE type=TYPE_STRING)	: m_int(0),m_nType(type)			{ ASSERT(type==TYPE_STRING || type==TYPE_SYMBOL); ASSERT(str); m_ptr=new MyString(str); }
...
//	operator MyString()	const								{ ASSERT(m_ptr); return *m_ptr; }
	operator const MyString&()	const						{ ASSERT(m_ptr); return *m_ptr; }
...
protected:
	MyString*		m_ptr;					// NULL: int type or no init
	int				m_int;
